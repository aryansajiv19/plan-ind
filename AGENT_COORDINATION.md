...
- 2026-09-07: **§23 — dark is the identity, and the mono-ink flattening is solved.** Owner: *"i think i like the dark theme more"*, meeting their own condition for making it the standard. Spec: `design-system/SPECS.md` §23. Mock: `design-system/mocks/energy-dark-v1.html` (audited in-browser: **0 text failures, every component boundary ≥3:1 on both sides**).
  ...
  - **§21's accent re-derived, not ported** — brown is 2.87/1.91 in dark and simply dies. `--accent: #BF977D` (tan +25% toward white) clears body text on **both** dark grounds (4.54 / 6.84), so §21 keeps one rule instead of growing a per-surface exception. Flagged in §23.6 as the one judgement call worth challenging.
  ...
  - **Not decided, flagged in §23.6:** light mode's fate, and the dark focus ring (§19.7's graphite inset is invisible on a near-black card).
