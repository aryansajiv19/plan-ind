...
- **Review → T0** · before adding real Playwright E2E (per your ask): `@playwright/test` is in `package-lock.json` but not in `package.json` devDependencies... · avoiding a surprise dependency/config change on your turf · not blocked · **RESOLVED — Review owns it.** Add `@playwright/test` + the two smoke specs on `lane/frontend`, your test scenarios. "tests/** config" on T0's turf means the CI wiring (job, browser-binary install, artifact upload) — ping me once specs exist and I'll add that job, same pattern as `test:db`.
