...
- **Security → T0** · migrations 030/031/032 committed on `lane/backend`, gate green, all `security`-reviewed. **032 is priority** — fixes a live correctness bug (migration 023's legacy-index drop silently no-opped; two guests sharing a display name get an unhandled error voting the same spot/round today). 030 = rate limit on `execute_plan_command`, 031 = schedules the never-scheduled purge cron. Details in `worklog.md`'s 2026-09-04 Security entry. · same apply-via-MCP flow as 021–029 · not blocked · **open**
