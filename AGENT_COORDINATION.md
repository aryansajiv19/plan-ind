...
- 2026-09-04: **Load-testing to real scale (thousands of concurrent users).** First pass topped out at n=15 (GoTrue's live anonymous-signup rate limit). This pass moved to a **local Supabase stack** (`npx supabase start`, Docker) instead of fighting that limit further — no rate limit locally, and it also closed the other first-pass gap (no permanent-account self-serve against the live project). Copied the real curated catalog from the live project into the local mirror (this is what surfaced the bug above), seeded 50 plans (not one — "thousands of users" means many people across many small plans, not one huge plan), minted 2,500 real permanent test accounts with full `@supabase/ssr` cookie sessions (0 failures on the final run, after fixing a local-GoTrue concurrency ceiling around n≈1,200-2,100 by throttling batch size). New scale scenarios (`scripts/load/scale.mjs`) for `vote-scale`/`rsvp-scale`/`plan-create-scale`/`spot-deal-scale` — the latter two genuinely new, blocked entirely in the first pass. Smoke-tested clean at n=10 each; full-scale numbers not yet run — the critical bug took priority the moment it surfaced. Next: run the real numbers now that both bugs are fixed.
