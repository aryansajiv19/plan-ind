...
Ranked by `docs/archive/DESIGN_DIAGNOSIS.md`'s corrected finding: the app doesn't feel like
...
| # | Task | Why | Size |
|---|---|---|---|
| **P1.1** | **Venue photography pipeline.** `docs/PLACES_INGESTION_SCOPE.md` is already written — read it, don't re-derive it. 6 photos across 82 venues is the single largest gap in the product. Free sources only; no paid Places API (owner's call, 2026-09-04). | Everything below #2 is downstream of this | L |
...
| Weather | **Not built, but free and unblocked** — Open-Meteo, verified working with no API key (`docs/archive/NEXT_AGENT.md`), independent of the AI/B3 blocker. Real add. |
...
in `docs/PLACE_IMPORT_ARCHITECTURE.md` (dated 2026-08-07) and its schema is live
...
Migrated from `docs/archive/NEXT_AGENT.md` §4.4, 2026-09-04 — real, unassigned, not
...
The layout direction for step 2/7's identified gap (`docs/PRODUCT_FLOW.md`) —
