...
export default function DemoPlanningTools({ view, onStartPlan }: { view: ToolView; onStartPlan: () => void }) {
  ...
  function saveMemoryPhoto(file: File) { fileAsDataUrl(file, (image) => { setMemoryPhoto(image); setPlan((current) => ({ ...current, memoryPhotoDataUrl: image })); }); }
  ...
  if (view === "been") return <section className="demo-tool-panel demo-memory-panel" aria-labelledby="memory-title"><p className="home-section-kicker">After the plan</p><h2 id="memory-title">Keep the good part.</h2><p>Turn a finished outing into a small shared memory. Photos stay on this device in demo mode.</p><label className="demo-tool-field"><span>What should you remember?</span><textarea value={plan.memoryNote} onChange={(event) => setPlan((current) => ({ ...current, memoryNote: event.target.value.slice(0, 280) }))} placeholder="The table, the joke, the place we want to return to…" maxLength={280} /></label><label className="demo-memory-upload"><input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) saveMemoryPhoto(file); }} />{memoryPhoto ? <Image src={memoryPhoto} alt="Memory preview" fill unoptimized sizes="(max-width: 760px) 100vw, 50vw" /> : <strong>Add a photo from the outing</strong>}</label><button type="button" className="demo-primary-action" onClick={() => setPlan((current) => ({ ...current, stage: "completed" }))}>Save memory</button></section>;
