...
export default function DemoPlanningTools({ view, onStartPlan }: { view: ToolView; onStartPlan: () => void }) {
  ...
  return <>
    <section className="demo-tool-panel demo-reminder-panel" aria-labelledby="reminder-title"><p className="home-section-kicker">Your plans</p><h2 id="reminder-title">A quiet nudge at the right time.</h2><p>{plan.reminderAt && plan.eventTime ? `Reminder set for ${new Date(plan.eventTime).toLocaleString()}.` : "Choose a plan time and turn on a local reminder when you are ready."}</p><label className="demo-check"><input type="checkbox" checked={Boolean(plan.reminderAt)} onChange={(event) => setPlan((current) => ({ ...current, reminderAt: event.target.checked ? current.eventTime : null }))} /> Local reminder enabled</label><button type="button" className="demo-primary-action" disabled={!plan.reminderAt && !plan.eventTime} onClick={() => setPlan((current) => ({ ...current, reminderAt: current.reminderAt ? null : current.eventTime }))}>{plan.reminderAt ? "Turn reminder off" : "Turn reminder on"}</button>{!plan.reminderAt && !plan.eventTime && <p className="demo-tool-saved">Set a plan time on the Friends tab first — the reminder follows it.</p>}</section>
  </>;
}
